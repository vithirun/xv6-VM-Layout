...
fetchint(struct proc *p, uint addr, int *ip)
{
  ...
  if(addr < 0x2000)
    return -1;
  if(((addr >= p->sz) || ((addr + 4) > p->sz)) && (addr < (USERTOP - (p->stk_sz * PGSIZE))))
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if(addr >= USERTOP || addr < 0x2000)
    return -1;
  if(((addr >= p->sz) && (addr < (USERTOP - (p->stk_sz * PGSIZE)))))
    return -1;
  *pp = (char*)addr;
  if(addr < p->sz)
    ep = (char*)p->sz;
...
argptr(int n, char **pp, int size)
{
  ...
  if((uint)i < 0x2000)
    return -1;
  if((((uint)i >= proc->sz) || ((uint)(i + size) > proc->sz)) && ((uint)i < (USERTOP - (proc->stk_sz * PGSIZE))))
    return -1;
  *pp = (char*)i;
  return 0;
}
